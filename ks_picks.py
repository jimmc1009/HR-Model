...
# Signal thresholds — loosened to generate more signals
OVER_SCORE       = 8.0
OVER_EDGE        = 0.4
LEAN_OVER_SCORE  = 5.0
LEAN_OVER_EDGE   = 0.15
...
def score_pitcher_whip(v: float) -> float:
   """Lower WHIP = more efficient pitcher = more Ks per inning."""
   if v <= 0: return 0.0
...
def score_pitcher_bb_pct(v: float) -> float:
   """Lower BB% = more strikes thrown = more K opportunities."""
   if v <= 0: return 0.0
   if v <= 4.0:  return  0.8
   if v <= 6.0:  return  0.4
   if v <= 8.0:  return  0.1
...
def compute_ks_score(row: pd.Series, lineup_k_stats: Dict[str, dict]) -> tuple:
   """Returns (ks_score, score_breakdown_str)"""
   ...
   breakdown_parts = []
   if s_k_pct:    breakdown_parts.append(f"K%:{s_k_pct:+.1f}")
   if s_swstr:    breakdown_parts.append(f"SwStr:{s_swstr:+.1f}")
   if s_chase:    breakdown_parts.append(f"Chase:{s_chase:+.1f}")
   if s_k9:       breakdown_parts.append(f"K/9:{s_k9:+.1f}")
   if s_velo:     breakdown_parts.append(f"Velo:{s_velo:+.1f}")
   if s_k21:      breakdown_parts.append(f"21d:{s_k21:+.1f}")
   if s_avg_ip:   breakdown_parts.append(f"IP:{s_avg_ip:+.1f}")
   if s_whip:     breakdown_parts.append(f"WHIP:{s_whip:+.1f}")
   if s_bb_pct:   breakdown_parts.append(f"BB%:{s_bb_pct:+.1f}")
   if s_opp_k:    breakdown_parts.append(f"OppK:{s_opp_k:+.1f}")
   if s_park:     breakdown_parts.append(f"Park:{s_park:+.1f}")
   if s_trends:   breakdown_parts.append(f"Trend:{s_trends:+.1f}")
   if s_opener:   breakdown_parts.append(f"Opener:{s_opener:+.1f}")

   return total, " | ".join(breakdown_parts)
...
def log_picks(gc: gspread.Client, sheet_id: str, picks: pd.DataFrame) -> None:
   ...
   col_order = ["date", "rank", "pitcher_name", "team", "k_line",
...
def main() -> None:
   ...
   print(picks[["rank", "pitcher_name", "ks_score", "score_breakdown", "projected_k_calc",
                "k_line", "prop_signal", "confidence"]].to_string(index=False))
