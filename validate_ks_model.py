"""
validate_ks_model.py
Full workup for the strikeout (K) model — both directions (over AND under).
Analogous to validate_model.py for HR, adapted for over/under prop betting.

Because K props are two-sided, edge must be checked per DIRECTION: an over bet
...
Checks:
  ...
  2. SCORE TIER x DIRECTION: which ks_score tiers predict overs vs unders
...
Read-only. Uses over_hit/under_hit already graded in KS_All_Scores.
"""
import os, json, time
...
def main():
    ...
    ws=None
    for tab in ["KS_All_Scores","KS_AllScores","K_All_Scores"]:
        try: ws=sh.worksheet(tab); break
        except Exception: continue
    if ws is None:
        print("  Could not find KS_All_Scores tab. Check the tab name."); return
    vals=wr(lambda:ws.get_all_values())
    df=pd.DataFrame(vals[1:],columns=vals[0])
    df=df[df.apply(lambda r:any(str(v).strip() for v in r),axis=1)].reset_index(drop=True)

    for c in ["ks_score","k_line","over_odds","under_odds"]:
    ...
    r["tier"]=r["ks_score"].apply(ks_tier)
    print(f"  {len(r)} resolved K props\n")
    ...
    for t in ["8+","6-8","4-6","2-4","0-2","<0"]:
        sub=r[r["tier"]==t]
        if len(sub)<MIN_SAMPLE: continue
        print(f"  {t:<6} {len(sub):>4} {sub['over'].mean()*100:>6.1f}% {sub['under'].mean()*100:>7.1f}%")
    print("  (High ks_score should predict OVERS; low/negative should predict UNDERS)")
    print()
    ...
    for t in ["8+","6-8","4-6","2-4","0-2","<0"]:
        for line in [4.5,5.5,6.5,7.5,8.5,9.5]:
            sub=wl[(wl["tier"]==t)&(wl["k_line"]==line)]
            if len(sub)<MIN_SAMPLE: continue
    ...
    if good:
        ...
        print("     the K model isn't finding edges in either direction.")
