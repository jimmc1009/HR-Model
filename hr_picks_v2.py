...
def write_picks_to_sheet(gc: gspread.Client, sheet_id: str, picks: pd.DataFrame) -> int:
    sh = gc.open_by_key(sheet_id)
    try:
        ws = sh.worksheet("Top_HR_Picks_v2")
        ws.clear()
    except gspread.WorksheetNotFound:
        ws = sh.add_worksheet(title="Top_HR_Picks_v2", rows=100, cols=55)
...
def log_all_scores(gc: gspread.Client, sheet_id: str, combined: pd.DataFrame) -> None:
    ...
    try:
        ws       = sh.worksheet("HR_All_Scores_v2")
        existing = pd.DataFrame(ws.get_all_records())
    except gspread.WorksheetNotFound:
        ws       = sh.add_worksheet(title="HR_All_Scores_v2", rows=10000, cols=40)
        existing = pd.DataFrame()
