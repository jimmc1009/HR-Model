...
def prepare_combined(
    ...
    pitcher_rename = {
        ...
        "vs_lhh_ip":                 "pitcher_vs_lhh_ip",
        "vs_rhh_ip":                 "pitcher_vs_rhh_ip",
        "hr9":                       "pitcher_hr9",
        "fb_rate_allowed":           "pitcher_fb_rate_allowed",
        "babip_allowed":             "pitcher_babip_allowed",
        "barrel_pct_7d":             "pitcher_barrel_7d",
        "barrel_pct_14d":            "pitcher_barrel_14d",
        "hr_7d":                     "pitcher_hr_7d",
        "hr_14d":                    "pitcher_hr_14d",
        "velo_trend":                "pitcher_velo_trend",
    ...
    pitcher_join_cols = [c for c in [
        ...
        "pitcher_hr9", "pitcher_fb_rate_allowed", "pitcher_babip_allowed",
        "pitcher_barrel_7d", "pitcher_barrel_14d", "pitcher_hr_7d",
        "pitcher_hr_14d", "pitcher_velo_trend",
...
def log_all_scores(gc: gspread.Client, sheet_id: str, combined: pd.DataFrame) -> None:
    ...
    for _, row in sorted_df.iterrows():
        new_rows.append({
            ...
            "platoon":                str(row.get("platoon_score", "")),
            "pitch":                  str(row.get("pitch_matchup_score", "")),
            "combined":               str(round(safe_float(row.get("platoon_score", 0)) + safe_float(row.get("pitch_matchup_score", 0)), 2)),
            ...
            "vs_lhp_iso":             str(row.get("vs_lhp_iso", "")),
            "vs_rhp_iso":             str(row.get("vs_rhp_iso", "")),
            "vs_lhp_hr_rate":         str(row.get("vs_lhp_hr_rate", "")),
            "vs_rhp_hr_rate":         str(row.get("vs_rhp_hr_rate", "")),
            "vs_lhp_barrel_pct":      str(row.get("vs_lhp_barrel_pct", "")),
            "vs_rhp_barrel_pct":      str(row.get("vs_rhp_barrel_pct", "")),
            ...
            "pitcher_hr9":            str(row.get("pitcher_hr9", "")),
            "pitcher_vs_lhh_hr9":     str(row.get("pitcher_vs_lhh_hr9", "")),
            "pitcher_vs_rhh_hr9":     str(row.get("pitcher_vs_rhh_hr9", "")),
            "pitcher_vs_lhh_ip":      str(row.get("pitcher_vs_lhh_ip", "")),
            "pitcher_vs_rhh_ip":      str(row.get("pitcher_vs_rhh_ip", "")),
            "pitcher_fb_rate_allowed": str(row.get("pitcher_fb_rate_allowed", "")),
            "pitcher_babip_allowed":  str(row.get("pitcher_babip_allowed", "")),
            "pitcher_barrel_7d":      str(row.get("pitcher_barrel_7d", "")),
            "pitcher_hr_7d":          str(row.get("pitcher_hr_7d", "")),
            "pitcher_velo_trend":     str(row.get("pitcher_velo_trend", "")),
